...
class OGPart {
   ...
   public:
    ...
    void partition() {
        ...
        for (int i = 1; i <= vertex_count; i++) {
            ...
            find_sub_part_and_build_sub_graph(cur_vertex_id, adj);
        }
        // Clean up
    ...
    void verify() {
        ...
        cout << "FinalVerify: ";
        cout << fixed << setprecision(2) << "CUTTANA," << dataset_name << ","
             << (result.is_vertex_balanced ? "VB" : "EB") << ","
             << ((double)(mx_edge) / ((double)edge_count / part_count)) << ","
             << (double)(mx) / ((double)vertex_count / part_count) << ","
             << (int)(result.program_timer.get_total_time() / 1e9) << ","
             << ((double)edge_cut / edge_count * 100.0) << ","
             << (sum_rep_fac * 1.0 / (vertex_count * result.part_count)) * 100.0
             << endl;
    }
   ...
   private:
    ...
    void find_sub_part_and_build_sub_graph(int vid, vector<int> &adj) {
        ...
        build_sub_partition_graph(vid, adj_state);
    };
    ...
    inline void evict_buffer(int vid = -1) {
        ...
        // TODO: Use move?
        find_sub_part_and_build_sub_graph(vid, adj);
    ...
    inline double get_balance_score(int partition, int parent_partition) {
        ...
        if (IS_VERTEX_BALANCED) {
            ...
            if (!partition_for_sub)
                ...
                return 0;
                // return alpha[partition_for_sub] * SUB_GAMMA *
                //        pow((1.0 * vertex_count + mu * 1.0 * edge_count)
                //        / 2.0,
                //            SUB_GAMMA - 1);
            }
        }
